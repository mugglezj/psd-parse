...
function default_1(textData) {
    const txt = textData['Txt '];
    const engineData = textData.EngineData;
    const fonts = getFonts(engineData.ResourceDict.FontSet);
    return getTextDetail(engineData.EngineDict.StyleRun.RunArray, engineData.EngineDict.StyleRun.RunLengthArray, fonts, txt);
    ...
    function getTextDetail(StyleArr, cutsArr, fonts, text) {
        ...
        function getColor(style) {
            ...
            var r = ~~(color[1] * 255), g = ~~(color[2] * 255), b = ~~(color[3] * 255);
            return 'rgb(' + r + ',' + g + ',' + b + ')';
        }
        ...
        function getKerning(style) {
            ...
            var fontSize = style.FontSize, value = style.Kerning;
            return ~~(fontSize * value / 1000) + 'px';
        }
        ...
        function getTextDecoration(style) {
            var underline = style.Underline, linethrough = style.Strikethrough;
            if (underline)
                return 'underline';
            if (linethrough)
                return 'line-through';
